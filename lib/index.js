...
async function isStorageReady () {
	return (
		await Fs.pathExists(allDir) &&
		await Fs.pathExists(bloomDir) &&
		await Fs.pathExists(infoDir) &&
		await Fs.pathExists(taggedDir) &&
		await Fs.pathExists(untaggedDir)
	);
}

async function ensureStorageIsReady () {
	await Fs.ensureDir(allDir);
	await Fs.ensureDir(bloomDir);
	await Fs.ensureDir(infoDir);
	await Fs.ensureDir(taggedDir);
	await Fs.ensureDir(untaggedDir);
}

async function addFile (srcPath, tags) {
	await ensureStorageIsReady();
